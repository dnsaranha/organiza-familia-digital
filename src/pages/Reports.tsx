...
import { PieChart, Pie, Cell, ResponsiveContainer, Legend, Tooltip, BarChart, XAxis, YAxis, CartesianGrid, Bar } from 'recharts';
...
const ReportsPage = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      setLoading(true);
      try {
        const { data, error } = await supabase.from('transactions').select('id, type, category, amount, date');
  ...
  const expenseByCategory = useMemo(() => {
    const expenseData = filteredTransactions.filter(t=>t.type === 'expense').reduce((acc, t) => {
                    ...
                    {filteredTransactions.map((t) => {
                      ...
                      return (
                        <TableRow key={t.id}>
                          ...
                          <TableCell className={cn('text-right font-medium', t.type === 'income' ? 'text-[#22c55e]' : 'text-[#ef4444]')}>
